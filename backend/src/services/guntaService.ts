...
export const createGunta = async (data: { name: string; description?: string }) => {
...
export const getGuntas = async (search?: string) => {
  ...
  return Gunta.find(filter).sort({ name: 1 });
};

export const getGuntaById = async (id: string) => {
  const gunta = await Gunta.findById(id);
  if (!gunta) throw new AppError('Gunta not found', 404);
  return gunta;
};

export const updateGunta = async (id: string, data: { name?: string; description?: string }) => {
